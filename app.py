...
def prediction(username):
    ...
    if username in MAN.select_column('username'):
        ...
        _b = _a['id'].values[0]
        _c = _a['name'].values[0]
        # print('Hello,',c,', here are some users for you to follow...')
        return jsonify(names=list(USER_MODEL.recommend(users=[_b], k=3)['name']))
        # print("Looks like you are a new user\n Here are some people to follow")
        # print(model.recommend(users=[4924], k=3))
